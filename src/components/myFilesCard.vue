    ...
    <v-card :class="{'loading-oppacity' : isLoadingData }" class="mx-auto" max-width="750">
      ...
      <v-list three-line>
        <template v-for="(item, index) in items">
          ...
          <v-list-item link :key="item.title" @click="getSpecificFile(item.id)">
            <v-list-item-avatar>
      ...
      <v-pagination v-model="pagination.page" :length="pagination.total" @input="fetchData" circle></v-pagination>
    </v-card>
    <v-progress-circular
      v-if="isLoadingData"
      :size="100"
...
export default {
  ...
  methods: {
    ...
    getSpecificFile(fileId) {
      this.$router.push(`MyFiles/${fileId}`);
    }
  },
  created() {
    this.fetchData();
